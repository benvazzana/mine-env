...
class RRT:
    ...
    def update(self, parent, agent_loc):
    ...
    def has_nearby_node(self, cur_node, agent_pos):
        for node in cur_node.adjacent_nodes:
    ...
    def is_reachable(self, pos1, pos2, min_dist=None, known=True):
        path = astar.shortest_path(self.layout, tuple(pos1), tuple(pos2), known=True)
        if path is None:
            return False
        if min_dist is None:
            return True
